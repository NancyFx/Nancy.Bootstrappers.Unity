...
assemblyinfo :version => [:clean] do |asm|
	...
	asm.description = "An Unity Bootstrapper for the Nancy web framework"
...
zip :package => [:publish] do |zip|
    ...
    zip.output_file = "Nancy.Bootstrappers.Unity-Latest.zip"
...
task :nuget_package => [:publish] do
    ...
    nuspecs = FileList["src/nancy.bootstrappers.unity/*.nuspec"]
    ...
    nuspecs.each do |nuspec|
        update_xml nuspec do |xml|
            ...
            xml.root.elements["metadata/licenseUrl"].text = "https://github.com/NancyFx/Nancy.Bootstrappers.Unity/blob/master/license.txt"
